...
export function BomsClient({
  ...
  return (
    <div>
      <InfoHero
        eyebrow="BOM DIFF"
        value={diff.length === 0 ? "No comparison yet" : `${diff.length} change${diff.length === 1 ? "" : "s"} detected`}
        supporting={
          diff.length === 0
            ? "Import both the current and proposed BOM to compute it."
            : "Between the current and proposed BOM, computed deterministically."
        }
      />
      <p className="mt-3 text-sm text-ink-soft">
        Every change below is computed deterministically — nothing is auto-inferred, and a removed+added pair
        only becomes &quot;replaced&quot; through an explicit pairing action. Import the current and proposed BOM
        below to get started (.xlsx, .xls, or .csv, up to {MAX_IMPORT_FILE_SIZE_LABEL} each).
      </p>
      ...
      <div className="mt-6">
        {diff.length === 0 ? (
          <EmptyState
            title="No BOM comparison yet"
            body="This diff is what identifies which parts this engineering change actually affects — everything downstream (mapping, exposure, mitigation) starts from it. Import both the current and proposed BOM above to compute it."
          />
        ) : (
