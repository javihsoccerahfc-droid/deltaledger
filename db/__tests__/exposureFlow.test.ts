...
async function setUpEcWithExposure() {
  ...
  const poCsv =
    ...
    `PO-Z,Zenith Supply,${partNumber},1000,15,USD,2026-09-01`;
  await poRepo.savePurchaseOrderImport(
    ec.id,
    await parseCsvFile(new File([poCsv], "po.csv", { type: "text/csv" })),
    "po.csv",
    partDataOwner.id
  );
