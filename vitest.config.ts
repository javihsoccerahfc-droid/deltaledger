  ...
  test: {
    ...
    projects: [
      ...
      },
      {
        ...
        test: {
          name: "components",
          // V2 introduces a real, reusable component library for the first time in this
          // project -- these are jsdom-rendered component tests (React Testing Library),
          // distinct from the plain-Node "unit" project above (which covers pure domain
          // logic with no DOM). Kept as a separate project rather than folding into "unit"
          // because jsdom environment setup only makes sense for files that render React.
          environment: "jsdom",
          include: ["src/components/**/*.test.tsx"],
          setupFiles: ["./src/testSetup/setupComponentTests.ts"],
        },
      },
      {
        extends: true,
